...
/**
 * router.php (the "guestbook" -- session cookie WITHOUT HttpOnly)
 *
 * Module 13: Cookie & Session Hardening.
 *
 * PHP equivalent of node/vulnerable-cookie/server.js. Comments persist
 * to comments.json for the same reason earlier PHP files did.
 *
 ...
 *
 * DO NOT copy this file's setcookie() call into real code.
 * php/fixed-cookie/router.php in this same module shows the real fix.
 */
...
function render_guestbook(array $comments): string
{
    ...
    return "<!DOCTYPE html><html><head><meta charset=\"utf-8\"><title>Module 13 Guestbook (vulnerable cookie)</title></head>"
        ...
        . "<h1>Module 13 &mdash; Guestbook (PHP, session cookie WITHOUT HttpOnly)</h1>"
...
if (!isset($_COOKIE['session'])) {
    // THE VULNERABILITY: no httponly flag -- readable by any script on this page
    setcookie('session', 'SECRET-abc123', ['path' => '/']);
}
